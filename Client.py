...
class Client:
    ...
    async def reconnect(self):
        ...
        for attempt in range(max_retries):
            logging.info(f"Attempting to reconnect (attempt {attempt + 1}/{max_retries})...")
            try:
    ...
    async def handle_disconnect(self):
        if not self.is_shutting_down:
            ...
            logging.info("Client is shutting down. Not attempting to reconnect.")
                ...
                if fingerprint:
                    ...
                    if self.rsa.verify(serialization.load_pem_public_key(sender_public_key), data_to_verify.encode('utf-8'), signature):
                        if self.check_counter(message, fingerprint):
                            ...
                            logging.warning(f"Invalid counter for message from {self.encode_fingerprint(fingerprint)}")
                    else:
                        logging.warning(f"Invalid signature for message from {self.encode_fingerprint(fingerprint)}")
                else:
                    logging.warning("Could not extract fingerprint from message")
    ...
    async def process_signed_data(self, data):
        try:
            if data["type"] == "hello":
                ...
                logging.warning(f"Unknown inner message type: {data['type']}")
        except KeyError as e:
            logging.error(f"Invalid inner message format: {e}")
        except Exception as e:
            logging.error(f"Error processing inner message: {e}")
    ...
    def get_fingerprint(self, public_key=None):
        ...
        try:
            ...
            logging.error(f"Error generating fingerprint: {e}")
            return None
    ...
    async def handle_hello(self, data):
        try:
            ...
            logging.info(f"Received hello from {fingerprint}")
        except Exception as e:
            logging.error(f"Error handling hello message: {e}")
    ...
    async def process_chat(self, data):
        try:
            ...
            if client_public_key not in encrypted_symm_keys:
                logging.warning("Received chat message not intended for this client")
                return
            ...
            logging.error(f"Invalid chat message format: {e}")
        except ValueError as e:
            logging.error(f"Error decoding base64 data: {e}")
        except InvalidSignature:
            logging.error("Invalid signature in chat message")
        except Exception as e:
    ...
    async def handle_public_chat(self, data):
        ...
        try:
            ...
            logging.error(f"Invalid public chat message format: {e}")
        except Exception as e:
            logging.error(f"Error handling public chat message: {e}")
    ...
    def extract_fingerprint(self, message):
        ...
        if data['type'] == 'hello':
            ...
            try:
                ...
                logging.error(f"Error decrypting chat message (extract): {e}")
                return None
    ...
    async def send_hello(self):
        ...
        try:
            ...
            logging.error(f"Error sending hello message: {e}")
    ...
    async def send_chat(self, recipient_encoded_fingerprints: list[str], message: str):
        try:
            ...
            logging.info(f"Chat message sent to {len(recipient_encoded_fingerprints)} recipients")
        except Exception as e:
    ...
    async def request_client_list(self):
        ...
        try:
            ...
            logging.error(f"Error requesting client list: {e}")
